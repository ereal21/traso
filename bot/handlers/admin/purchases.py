...
async def pirkimai_callback_handler(call: CallbackQuery):
    ...
    await bot.edit_message_text(
        t(lang, 'purchases_choose_date'),
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        reply_markup=purchases_dates_list(dates, lang),
    )
...
async def purchases_date_callback_handler(call: CallbackQuery):
    ...
    lang = get_user_language(user_id) or 'en'
    await bot.edit_message_text(
        t(lang, 'purchases_for_date', date=date),
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        reply_markup=purchases_list(purchases, date, lang),
    )
...
async def purchase_info_callback_handler(call: CallbackQuery):
    ...
    lang = get_user_language(user_id) or 'en'
    if not purchase:
        await call.answer(t(lang, 'purchase_not_found'), show_alert=True)
        return
    ...
    await bot.edit_message_text(
        ...
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        reply_markup=purchase_info_menu(purchase_id_int, date, lang),
    )
